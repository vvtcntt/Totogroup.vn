    ...
    public class ProductadController : Controller
    {
        ...
        public ActionResult Create(string id)
        {
            ...
            if (ClsCheckRole.CheckQuyen(4, 1, int.Parse(Request.Cookies["Username"].Values["UserID"])) == true)
            {
                ...
                var Listgroupprice = db.tblGroupPrices.Where(p => p.Active == true).OrderBy(p => p.Ord).ToList();
                var lstprice = new List<SelectListItem>();
                lstprice.Clear();
                foreach (var item in Listgroupprice)
                {

                    lstprice.Add(new SelectListItem { Text = item.GroupName, Value = item.id.ToString() });
            ...
            if (ModelState.IsValid)
            {
                if (collection["drMenu"] != "" || collection["drMenu"] != null)
                {
                    ...
                    if (URL == true)
                    {
                        ...
                        clsSitemap.UpdateSitemap("1/"+tblproduct.Tag, id.ToString(), "Product");
        ...
        public JsonResult PriceSale(string id)
        {
            int nid = int.Parse(id);
            int Sale = 0;
            var GroupPrice = db.tblGroupPrices.First(p => p.id == nid);
            Sale = int.Parse(GroupPrice.Sale.ToString());

            return Json(new { Sale = Sale });
